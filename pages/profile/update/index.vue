  ...
  <v-row justify="center">
    <v-col cols="10" sm="6">
      <v-card class="my-4" elevation="2">
        <v-card-title> عدل بياناتك من هنا </v-card-title>
        <v-card-text>
          <v-form v-model="valid" @submit.prevent="updateUser">
            ...
            <v-radio-group v-model="form.planet" :rules="planetRules" row>
              <v-radio label="زحل" value="saturn"></v-radio>
              <img
                ...
                class="ml-2"
              />
              <v-radio label="المريخ" value="jupiter"></v-radio>
              <img
...
export default {
  ...
  methods: {
    ...
    async updateUser() {
      ...
      try {
        await this.$axios
          .post("/users/" + this.user.id, this.form)
          .then((res) => {
            try {
