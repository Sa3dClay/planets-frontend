<template>
  <v-card v-if="usersRequestedFriendship.length" class="my-4">
    ...
    <v-card-text>
      <v-list>
        <v-list-item
          v-for="(user, index) in usersRequestedFriendship"
          :key="user.id"
        >
          <v-list-item-content>
            {{ user.name }}
          ...
          <v-list-item-action>
            <v-btn
              ...
              @click.prevent="acceptFriendship(user.id, index)"
              >قبول</v-btn
            >
          </v-list-item-action>
          <v-list-item-action>
            <v-btn
              ...
              @click.prevent="denyFriendship(user.id, index)"
              >رفض</v-btn
            >
          </v-list-item-action>
        </v-list-item>
...
export default {
  ...
  methods: {
    async getUsersRequestedFriendship() {
      const res = await this.$axios.$get("/users/friends/requests");
    ...
    async denyFriendship(userId, index) {
      try {
        await this.$axios.$post("/users/friends/requests/deny/" + userId);
